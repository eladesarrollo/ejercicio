@php use function App\Helpers\isSelected; @endphp
@extends('layout')
@if(session('status'))
...
<div class="row">
    <div class="col-2"></div>
    <div class="col-8">
        <form class="row g-3" action="{{ route('asignacion.update', $elemento->id) }}" method="POST"
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" aria-label="Default select example" name="proyecto_id">
                        ...
                        @foreach($proyectos as $proyecto)
                            <option
                                value="{{$proyecto->id}}"
                                {{isSelected($proyecto->id, $elemento->proyecto_id)}}>
                                {{$proyecto->nombre}}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-select" aria-label="Default select example" name="cooperante_id">
                        ...
                        @foreach($cooperantes as $cooperante)
                            <option value="{{$cooperante->id}}"
                                {{isSelected($cooperante->id, $elemento->cooperante_id)}}>
                                {{$cooperante->nombre}}
                            </option>
                        @endforeach
                    </select>
